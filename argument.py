...
class CustomizedArguments:
    ...
    train_path: str = field(metadata={"help": "训练集。如果task_type=pretrain，请指定文件夹，将扫描其下面的所有jsonl文件"})
    dev_path: str = field(metadata={"help": "验证集。如果task_type=pretrain，请指定文件夹，将扫描其下面的所有jsonl文件"})
    test_path: str = field(metadata={"help": "测试集。如果task_type=pretrain，请指定文件夹，将扫描其下面的所有jsonl文件"})
